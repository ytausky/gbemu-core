...
impl<'a> InstrExecution<'a> {
    ...
    fn execute_microinstruction_on_tock(
        ...
        if let Some(byte_writeback) = &microinstruction.byte_writeback {
            ...
            match byte_writeback.dest {
                ...
                DataSel::PcH => self.regs.pc = self.regs.pc & 0x00ff | u16::from(byte) << 8,
                DataSel::PcL => self.regs.pc = self.regs.pc & 0xff00 | u16::from(byte),
    ...
    fn data(&self, data_sel: DataSel) -> u8 {
        match data_sel {
            ...
            DataSel::PcH => high_byte(self.regs.pc),
            DataSel::PcL => low_byte(self.regs.pc),
    ...
    fn alu_operand(&self, operand: AluOperand, data: u8) -> u8 {
        match operand {
            ...
            AluOperand::One => 0x01,
            AluOperand::PcH => high_byte(self.regs.pc),
            AluOperand::PcL => low_byte(self.regs.pc),
